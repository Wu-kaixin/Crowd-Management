...
疏散 / DBAct / density-DBAct 代码**不在** `main` 上。它们仅保存在 [`local-main-backup`](https://github.com/Wu-kaixin/Crowd-Management/tree/local-main-backup)，供复现使用。
...
可用 `git switch local-main-backup` 查看。新工作从 `main` 上的 `scripts/run_static_containment.py` 开始。
