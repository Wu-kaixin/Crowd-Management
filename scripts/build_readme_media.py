...
def build_media() -> None:
    ...
    for ax, (name, raw, guide_count, bins, seed) in zip(axes.ravel(), SCENARIOS):
        ...
        metrics = containment_summary(guide_points, crowd_points, boundary, coverage_radius=1.3, min_crowd_distance=safety_distance)
        summaries.append((name, metrics))
        cache.append((name, crowd_points, guide_points, boundary))
        ...
        ax.set_title(f"{name}: coverage {metrics['coverage_ratio']:.2f}, gap {metrics['max_boundary_gap']:.2f}")
        ax.set_aspect("equal")
        ax.set_xlim(5, 15)
        ax.set_ylim(2, 12)
        ax.grid(alpha=0.18)
    ...
    gaps = [item[1]["max_boundary_gap"] for item in summaries]
    ...
    ax2 = ax1.twinx()
    ax2.bar(x + 0.18, gaps, width=0.36, color="#e15759", label="max boundary gap")
    ax2.set_ylabel("Max boundary gap")
